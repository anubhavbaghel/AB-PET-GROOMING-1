...
export default function ReviewsManager() {
  ...
  // Helper: get the usable id string from a review doc (supports both legacy numeric id and MongoDB ObjectId)
  function getReviewId(r: any): string {
    return String(r._id || r.id || '')
  }

  // Helper: get the display id label
  ...
  async function approve(r: any) {
    ...
    if (!id || !confirm(`Approve this review?`)) return
  ...
  async function reject(r: any) {
    const id = getReviewId(r)
    if (!id || !confirm(`Reject this review?`)) return
  ...
  return (
    <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #E6D9F5', boxShadow: '0 4px 12px rgba(113, 88, 166, 0.05)', padding: '24px' }}>
      ...
      {loading ? <div style={{ padding: '40px', textAlign: 'center', color: '#7A7A7A' }}>Loading reviews...</div> : (
        reviews.length === 0 ? (
          <div style={{ padding: '40px', textAlign: 'center', color: '#7A7A7A' }}>No reviews yet.</div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
              <thead style={{ textAlign: 'left', background: '#F8F6FC', borderTop: '1px solid #E6D9F5', borderBottom: '2px solid #E6D9F5' }}>
                <tr>
                  <th style={{ padding: '12px 16px', color: '#4A4A4A', fontWeight: 600, width: '80px' }}>ID</th>
                  <th style={{ padding: '12px 16px', color: '#4A4A4A', fontWeight: 600, width: '160px' }}>Name</th>
                  <th style={{ padding: '12px 16px', color: '#4A4A4A', fontWeight: 600, width: '100px' }}>Rating</th>
                  <th style={{ padding: '12px 16px', color: '#4A4A4A', fontWeight: 600 }}>Review</th>
                ...
                {reviews.map((r: any, idx: number) => (
                  <tr key={String(r._id ?? r.id ?? idx)} style={{ borderBottom: '1px solid #E6D9F5' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      {/* New reviews use 'name', legacy reviews use 'customer_name' */}
                      <div style={{ color: '#2d2047', fontWeight: 500 }}>{r.name || r.customer_name || r.customer_id || '-'}</div>
                      <div style={{ fontSize: '12px', color: '#7A7A7A' }}>{r.customer_phone || ''}</div>
                    </td>
                    <td style={{ padding: '12px 16px', fontSize: '16px' }}>
                      {r.rating ? '★'.repeat(Number(r.rating)) + '☆'.repeat(5 - Number(r.rating)) : '-'}
                    </td>
                    {/* New reviews use 'message', legacy reviews use 'comment' */}
                    <td style={{ padding: '12px 16px', color: '#4A4A4A', fontStyle: 'italic' }}>"{r.message || r.comment || ''}"</td>
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button onClick={() => approve(r)} style={{ padding: '6px 12px', background: '#eefaf0', color: '#187a2e', border: '1px solid #bee3c6', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '12px' }}>Approve</button>
                        <button onClick={() => reject(r)} style={{ padding: '6px 12px', background: '#fff1f1', color: '#b22222', border: '1px solid #f0c4c4', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '12px' }}>Reject</button>
                        <button onClick={() => handleDelete(r)} style={{ padding: '6px 12px', background: '#fff', color: '#D32F2F', border: '1px solid #FFCDD2', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '12px' }}>Del</button>
                      </div>
                    </td>
                  </tr>
                ))}
